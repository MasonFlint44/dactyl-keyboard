...
class EngineConfig(BaseModel):
    engine: EngineType = Field("solid", alias="ENGINE")
...
class ShapeConfig(BaseModel):
    ...
    nrows: int = Field(5, description="key rows")
    ncols: int = Field(6, description="key columns")
    alpha: float = Field(pi / 12.0, description="curvature of the columns")
    beta: float = Field(pi / 36.0, description="curvature of the rows")
    centercol: int = Field(
        3,
        description="controls left_right tilt / tenting (higher number is more tenting)",
    )
    centerrow_offset: int = Field(
        3, description="rows from max, controls front_back tilt"
    )
    tenting_angle: float = Field(pi / 12.0, description="more precise tenting control")
...
class ThumbConfig(BaseModel):
    ...
    default_1U_cluster: Optional[bool] = Field(
        True, description="only used with default, makes top right thumb cluster key 1U"
    )
...
class TrackballInWallConfig(BaseModel):
    trackball_in_wall: bool = Field(
        False, description="Separate trackball option, placing it in the OLED area"
    )
    tbiw_ball_center_row: float = Field(
        0.2, description="up from cornerrow instead of down from top"
    )
    ...
    tbiw_oled_center_row: float = Field(
        0.75, description="offsets are from this position"
    )
...
class TrackballJsConfig(BaseModel):
    ...
    tbjs_translation_offset: Tuple[int, int, int] = Field(
        (0, 0, 10), description="applied to the whole assy"
    )
    tbjs_rotation_offset: Tuple[int, int, int] = Field(
        (0, 0, 0), description="applied to the whole assy"
    )
...
class TrackballConfig(BaseModel):
    ...
    trackball_Usize: float = Field(1.5, description="size for inner key near trackball")
    ball_side: BallSideType = "right"
    ball_diameter: float = 34.0
    ball_wall_thickness: int = Field(
        3, description="should not be changed unless the import models are changed."
    )
    ball_gap: float = 1.0
...
class ExperimentalConfig(BaseModel):
    pinky_1_5U: bool = Field(False, description="LEAVE AS FALSE, CURRENTLY BROKEN")
    first_1_5U_row: int = 0
    last_1_5U_row: int = 5
    # TODO: are below part of ShapeConfig instead?
    extra_width: float = Field(
        2.5, description="extra space between the base of keys - original=2"
    )
    extra_height: float = Field(1.0, description="original= 0.5")
    wall_z_offset: int = Field(
        15, description="length of the first downward_sloping part of the wall"
    )
    ...
    left_wall_x_offset: int = Field(
        12, description="specific values for the left side due to the minimal wall."
    )
    left_wall_z_offset: int = Field(
        3, description="specific values for the left side due to the minimal wall."
    )
    left_wall_lower_y_offset: int = Field(
        0, description="specific values for the lower left corner."
    )
    left_wall_lower_z_offset: int = 0
    wall_thickness: float = Field(
        4.5, description="wall thickness parameter used on upper/mid stage of the wall"
    )
    wall_base_y_thickness: float = Field(
        4.5, description="wall thickness at the lower stage"
    )
...
class FixedColumnStyleConfig(BaseModel):
    ...
    fixed_x: List[float] = Field([], description="relative to the middle finger")
    fixed_z: List[float] = Field(
        [], description="overrides the z portion of the column ofsets above"
    )
...
class SwitchHoleConfig(BaseModel):
    ...
    undercut_transition: float = Field(
        0.2, description="NOT FUNCTIONAL WITH OPENSCAD, ONLY WORKS WITH CADQUERY"
    )
    plate_file: Optional[str] = Field(None, description="Custom plate step file")
    plate_offset: float = 0.0


class BaseOledConfig(BaseModel):
    oled_mount_width: float = Field(12.5, description="width of OLED, plus clearance")
...
class UndercutOledConfig(BaseOledConfig):
    oled_mount_width: float = Field(15.0, description="width of OLED, plus clearance")
    oled_mount_height: float = Field(35.0, description="length of screen")
...
class SlidingOledConfig(BaseOledConfig):
    oled_mount_height: float = Field(25.0, description="length of screen")
    ...
    oled_thickness: float = Field(
        4.2, description="thickness of OLED, plus clearance.  Must include components"
    )
    oled_edge_overlap_end: float = Field(
        6.5, description="length from end of viewable screen to end of PCB"
    )
    ...
    oled_edge_overlap_thickness: float = Field(
        2.5, description="thickness of material over edge of PCB"
    )
    oled_edge_overlap_clearance: float = Field(
        2.5, description="Clearance to insert PCB before laying down and sliding."
    )
    oled_edge_chamfer: float = 2.0


class ClipOledConfig(BaseOledConfig):
    oled_mount_height: float = Field(39.0, description="length of screen")
    ...
    oled_mount_bezel_thickness: float = Field(
        3.5, description="z thickness of clip bezel"
    )
    oled_mount_bezel_chamfer: float = Field(
        2.0, description="depth of the 45 degree chamfer"
    )
...
class OledConfigs(BaseModel):
    undercut: UndercutOledConfig = Field(..., alias="UNDERCUT")
    sliding: SlidingOledConfig = Field(..., alias="SLIDING")
    clip: ClipOledConfig = Field(..., alias="CLIP")
...
class OledMountConfig(BaseModel):
    ...
    oled_configurations: OledConfigs
...
class BottomPlateConfig(BaseModel):
    ...
    base_thickness: float = Field(
        3.0, description="thickness in the middle of the plate"
    )
    ...
    base_rim_thickness: float = Field(
        5.0, description="thickness on the outer frame with screws"
    )
